import {
  Menu, Bell, Plus, Archive, Users, ChevronRight,
} from 'lucide-react'
...
// ─── Type helpers ─────────────────────────────────────────────────────────────
const L1 = (extra) => ({ fontFamily: quicksand, fontWeight: 600, fontSize: 20, lineHeight: '100%', ...extra })
const L2 = (extra) => ({ fontFamily: quicksand, fontWeight: 600, fontSize: 16, lineHeight: '20px', ...extra })
const L3 = (extra) => ({ fontFamily: inter, fontWeight: 400, fontSize: 13, lineHeight: '18px', ...extra })
const L4 = (extra) => ({ fontFamily: inter, fontWeight: 400, fontSize: 11, lineHeight: '18px', ...extra })
const L5 = (extra) => ({ fontFamily: inter, fontWeight: 500, fontSize: 12, lineHeight: '100%', ...extra })
const L6 = (extra) => ({ fontFamily: inter, fontWeight: 500, fontSize: 10, lineHeight: '100%', letterSpacing: '0.8px', textTransform: 'uppercase', ...extra })
...
export default function PatientHomePage() {
  return (
    /* Phone wrapper — centers on desktop, full screen on mobile */
    <div style={{ minHeight: '100dvh', background: '#e8e8ee', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '0' }}>
      <div style={{
        ...
        {/* ── NavBar ── */}
        <div style={{ height: 52, flexShrink: 0, display: 'flex', alignItems: 'center', paddingLeft: 12, paddingRight: 16, gap: 0, background: c.white }}>
          <Menu size={24} strokeWidth={1.75} color={c.textPri} style={{ flexShrink: 0 }} />
          <div style={{ flex: 1 }} />
        ...
        <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 100 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: '20px 16px 8px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {QUICK_ACTIONS.map((qa) => (
                  <button key={qa.label} style={{ ...card, flex: 1, height: 88, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8, padding: 6, cursor: 'pointer', background: c.white }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              {REQUESTS.map((r) => (
                <div key={r.title} style={{ ...card, height: 64, display: 'flex', alignItems: 'center', padding: '0 16px' }}>
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 2, overflow: 'hidden' }}>
                    <span style={{ fontFamily: quicksand, fontWeight: 600, fontSize: 14, lineHeight: 'normal', color: c.textPri, whiteSpace: 'nowrap' }}>{r.title}</span>
                    <span style={{ ...L4({ color: c.textSec }), whiteSpace: 'nowrap' }}>{r.meta}</span>
                  </div>
                  <div style={{ background: c.white, border: `1px solid ${c.border}`, borderRadius: 11, padding: '4px 10px', flexShrink: 0 }}>
        ...
        {/* ── Bottom sticky CTA ── */}
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '0 16px 32px', background: c.white }}>
          <button style={{
            width: '100%', height: 44,
            background: 'linear-gradient(180deg, #245dcf 0%, #122f69 100%)',
